# ZXCode 六核心工具与 LLM 工具循环任务

## 任务 1：建立工具契约与显式注册表

- **目标**：定义最小工具契约、调用上下文、结构化结果和注册表；显式注册六个内置工具，拒绝重名，并能导出 OpenAI-compatible 工具描述。
- **影响文件**：`zxcode/tools/__init__.py`、`zxcode/tools/base.py`、`tests/test_tool_runtime.py`。
- **依赖任务**：无。
...
- **影响文件**：`zxcode/tools/base.py`、`zxcode/tools/executor.py`、`tests/test_tool_runtime.py`。
- **依赖任务**：任务 1。
...
- **影响文件**：`zxcode/tools/files.py`、`tests/test_file_tools.py`。
- **依赖任务**：任务 1。
...
- **影响文件**：`zxcode/tools/search.py`、`tests/test_search_tools.py`。
- **依赖任务**：任务 1、任务 3。
...
- **影响文件**：`zxcode/tools/shell.py`、`zxcode/tools/base.py`、`tests/test_shell_tool.py`。
- **依赖任务**：任务 1、任务 2、任务 3。
- **参考资料定位**：`docs/spec.md` 的“命令执行与授权”；`zxcode/app.py:110-114` 的现有取消入口；Python `asyncio.create_subprocess_exec` 文档；Windows `taskkill /T` 行为说明。

## 任务 7：接入 SDK 流式工具调用与循环终止

- **目标**：让客户端在保留文本增量的同时获得 SDK 累积后的完整工具调用；实现“请求—执行工具—回传结果—再次请求”的循环，并把现有终止器迁入可导入的 ZXCode 模块复用。
- **影响文件**：`zxcode/client.py`、`zxcode/agent.py`、`zxcode/session.py`、`tests/test_client.py`、`tests/test_agent.py`、`tests/test_session.py`。
- **依赖任务**：任务 1、任务 2。
- **参考资料定位**：`zxcode/client.py:32-56` 的现有流式客户端；`zxcode/session.py:13-34` 的会话快照与提交；`agnent loop part/agent/loop/terminator.py:10-127` 的现有终止逻辑；Context7 `/openai/openai-python/v2.11.0` 中 `chat.completions.stream()`、`get_final_completion()` 和 tool message 文档。
...
- **目标**：在应用启动时显式构造注册表、六个工具、执行器和 Agent 循环；把 `ZXCodeApp.generate` 改为消费文本、工具状态、授权请求和最终结果，并保持现有快捷键、斜杠命令、状态栏和事务式会话语义。
- **影响文件**：`zxcode/app.py`、`zxcode/__main__.py`、`zxcode/session.py`、`tests/test_app.py`、`tests/test_main.py`、`README.md`。
- **依赖任务**：任务 3、任务 4、任务 5、任务 6、任务 7、任务 8。
- **参考资料定位**：`zxcode/app.py:42-48` 的依赖构造；`zxcode/app.py:66-83` 的提交入口；`zxcode/app.py:117-145` 的现有生成流程；`zxcode/__main__.py:9-20` 的启动入口；`zxcode/session.py:21-34` 的请求与提交边界。
